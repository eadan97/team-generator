...
from utils.players import get_average_stats_df, get_player_names, get_players_by_names
...
if "changed_stats" in st.session_state:
    del st.session_state.changed_stats
    del st.session_state.players  # Force reload of players to avoid stale data
...
selected_players = st.multiselect(
    ...
    options=get_player_names(st.session_state.players),
    # default=st.session_state.players["Name"].tolist(), # TODO: Discuss if this should be default or not
)
...
with st.expander("### Player Pair Restrictions"):
    ...
    if not edited_pairs_df.empty and st.button("Delete Selected"):
        to_delete = edited_pairs_df[edited_pairs_df["Delete?"]].index.tolist()
...
with col2:
    num_variations = st.number_input(
        "Number of variations", min_value=1, max_value=10, value=3, step=1, format="%d"
    )

if st.button("Generate Teams"):
    selected_players_dict = get_players_by_names(
        st.session_state.players, selected_players
    )
    player_stats_config = st.session_state.config["Player Stats"]
    player_stats_list = list(player_stats_config.keys())
    selected_players_df = get_average_stats_df(
        selected_players_dict, player_stats_config
    )
    ...
    if variations is None:
        ...
        for i, tab in enumerate(tabs):
            with tab:
                ...
                for i in range(
                    2
                ):  # TODO: change this to a max value of selected_var_list
                    st.subheader(f"Team {i + 1}")
                    team_df = selected_players_df[
                        [x == i for x in selected_var_list]
                    ].reset_index(drop=True)[
                        st.session_state.config["player_base_columns"]
                        + player_stats_list
                    ]
                    st.table(team_df)
                    teams_dfs.append(team_df[player_stats_list].sum())
